...
run_migration($conn, '010_add_translation_columns', function (mysqli $conn) {
    $conn->query("ALTER TABLE `categories`
        ...
        ADD COLUMN IF NOT EXISTS `title_sw`       varchar(150) DEFAULT NULL,
        ADD COLUMN IF NOT EXISTS `description_rw` text         DEFAULT NULL,
        ADD COLUMN IF NOT EXISTS `description_fr` text         DEFAULT NULL,
        ADD COLUMN IF NOT EXISTS `description_sw` text         DEFAULT NULL
    ");
    $conn->query("ALTER TABLE `subcategories`
        ...
        ADD COLUMN IF NOT EXISTS `name_sw` varchar(150) DEFAULT NULL
    ");
});

run_migration($conn, '011_seed_category_translations', function (mysqli $conn) {
    $t = [
        ['clothing',        'Imyenda',               'Vêtements',    'Nguo',               'Imyenda harimo amakabutura, amashati na tiresi',              'Vêtements incluant chemises, sweats et pantalons', 'Nguo kama ni nguo za kimwili, nguo za kuzungumzia, na nguo za kazi'],
        ['shoes',           'Inkweto',               'Chaussures',   'Viatu',              'Ubwoko bwose bw\'inkweto harimo za sport na casual',           'Tous types de chaussures sport et casual', 'Viatu vya kila aina, kama vile viatu vya michezo na viatu vya kawaida'],
        ['electronics',     'Ikoranabuhanga',        'Électronique', 'Vifaa vya kielektroniki', 'Ibikoresho bya tekinoloji nk\'amasaha, amafone na televiziyo', 'Appareils électroniques, montres, écouteurs et gadgets', 'Vifaa vya teknolojia kama vile sawa, vifaa vya kuzungumzia, na vifaa vya kuweka'],
        ['kitchen--dining', 'Igikoni',               'Cuisine',      'Jikoni',             'Ibikoresho by\'igikoni, ibikapu n\'amatafari',                'Essentiels de cuisine, vaisselle et sets de table', 'Vifaa vya jikoni, vyombo vya kuzungumzia, na seti za kuzungumzia'],
        ['cars',            'Imodoka',               'Voitures',     'Magari',              'Imodoka nshya na za kera, SUV na bike',                       'Voitures neuves et d\'occasion, SUV et accessoires', 'Magari mapya na zamani, SUV na magari mengi'],
        ['drinkware',       'Ibikoresho by\'kunywa', 'Boissons',    'Vyombo vya kunywa',  'Amacupa, ibikombe n\'ibikoresho by\'kunywa',                   'Bouteilles, tasses et accessoires de boisson', 'Vyombo vya kunywa kama vile chupa, kofia na vyombo vya kuweka maji'],
    ];
    $stmt = $conn->prepare("UPDATE `categories` SET title_rw=?, title_fr=?, title_sw=?, description_rw=?, description_fr=?, description_sw=? WHERE slug=?");
    foreach ($t as [$slug, $rw, $fr, $sw, $drw, $dfr, $dsw]) {
        $stmt->bind_param('sssssss', $rw, $fr, $sw, $drw, $dfr, $dsw, $slug);
        $stmt->execute();
    }
    $stmt->close();
});
...
run_migration($conn, '012_seed_subcategory_translations', function (mysqli $conn) {
    $t = [
        ['men',                   'Imyenda y\'Abagabo',       'Vêtements Hommes', 'Nguo za Wanaume'],
        ['clothes-women',         'Imyenda y\'Abagore',       'Vêtements Femmes', 'Nguo za Wanawake'],
        ['kids-clothes',          'Imyenda y\'Abana',         'Vêtements Enfants', 'Nguo za Watoto'],
        ['t-shirts',              'Amatisheti',               'T-Shirts', 'Tishoti'],
        ['hoodies-sweatshirts',   'Amahudie',                 'Sweats à capuche', 'Hudi na Sweater'],
        ['shirts-blouses',        'Amashati',                 'Chemises et Blouses', 'Shati na Blauzi'],
        ['jeans',                 'Amajeani',                 'Jeans', 'Jenzi'],
        ['trousers-chinos',       'Amakabutura',              'Pantalons et Chinos', 'Suruali na Chino'],
        ['dresses',               'Imideri',                  'Robes', 'Vestiti'],
        ['skirts',                'Amashati yabagore',        'Jupes', 'Sketi'],
        ['jackets-coats',         'Amakoti',                  'Vestes et Manteaux', 'Jaketi na Koti'],
        ['suits-blazers',         'Amasuti',                  'Costumes et Blazers', 'Suti na Bulezi'],
        ['shorts',                'Amashoti',                 'Shorts', 'Shoti'],
        ['activewear-sportswear', 'Imyenda ya siporo',        'Vêtements de sport', 'Nguo za Mchezo'],
        ['underwear-lingerie',    'Imyenda y\'imbere',        'Sous-vêtements', 'Nguo za Kando'],
        ['socks',                 'Amaporoho',                'Chaussettes', 'Soksi'],
        ['pyjamas-loungewear',    'Imyenda yo gusinzira',     'Pyjamas', 'Pajama na Nguo za Kuzungumzia'],
        ['abayas-modest-wear',    'Imyenda y\'idini',         'Vêtements modestes', 'Nguo za Kujichwa'],
        ['school-uniforms',       'Imyenda y\'ishuri',        'Uniformes scolaires', 'Nguo za Shule'],
        ['traditional-cultural',  'Imyenda y\'amasano',       'Vêtements traditionnels', 'Nguo za Kitamaduni'],
        ['for-everyone-shoes',    'Inkweto za bose',          'Chaussures pour tous', 'Viatu vya Kila Mtu'],
        ['mens-shoes',            'Inkweto z\'Abagabo',       'Chaussures Hommes', 'Viatu vya Wanaume'],
        ['womens-shoes',          'Inkweto z\'Abagore',       'Chaussures Femmes', 'Viatu vya Wanawake'],
        ['kids-shoes',            'Inkweto z\'Abana',         'Chaussures Enfants', 'Viatu vya Watoto'],
        ['sneakers',              'Inkweto z\'imikino',       'Baskets', 'Sneakers'],
        ['running-shoes',         'Inkweto zo gutera',        'Chaussures de course', 'Viatu vya Kukimbia'],
        ['sport-shoes',           'Inkweto za siporo',        'Chaussures de sport', 'Viatu vya Mchezo'],
        ['casual-shoes',          'Inkweto za buri munsi',    'Chaussures casual', 'Viatu vya Kawaida'],
        ['formal-shoes',          'Inkweto z\'ibikorwa',      'Chaussures formelles', 'Viatu vya Ofisi'],
        ['boots',                 'Amabutu',                  'Bottes', 'Buti'],
        ['sandals-flip-flops',    'Amasandali',               'Sandales', 'Sandali'],
        ['heels',                 'Inkweto z\'intako',        'Talons', 'Viatu vya Kichwa'],
        ['loafers-slip-ons',      'Inkweto zidafunga',        'Mocassins', 'Viatu vya Lofa'],
        ['mobile-devices',        'Ibikoresho bya telefone',  'Appareils mobiles', 'Vifaa vya Simu'],
        ['smartphone',            'Telefone ngiri',           'Smartphone', 'Smartphone'],
        ['keypad-phone',          'Telefone y\'inkingi',      'Téléphone à touches', 'Simu ya Button'],
        ['tablets',               'Ibikoresho bya tablete',   'Tablettes', 'Tableti'],
        ['laptops',               'Mudasobwa ngufi',          'Ordinateurs portables', 'Kompyuta ya Mkono'],
        ['desktop-computers',     'Mudasobwa nini',           'Ordinateurs de bureau', 'Kompyuta ya Desktopi'],
        ['smartwatches',          'Isaha ya telefone',        'Montres connectées', 'Saa ya Smartwatch'],
        ['watches',               'Amasaha',                  'Montres', 'Saa'],
        ['headphones-earbuds',    'Amahuzikijwe ijwi',        'Écouteurs', 'Hedfoni'],
        ['bluetooth-speakers',    'Amabara y\'ijwi',          'Enceintes Bluetooth', 'Spika'],
        ['tv-monitors',           'Televiziyo',               'TV et Moniteurs', 'TV na Monita'],
        ['cameras-photography',   'Kamera',                   'Appareils photo', 'Kamera'],
        ['phone-accessories',     'Ibikoresho bya telefone',  'Accessoires téléphone', 'Vifaa vya Simu'],
        ['chargers-cables',       'Ibikoresho byo kuzungurura bateri na makaburi', 'Chargeurs et Câbles', 'Chaja na Kabeli'],
        ['power-banks',           'Bateri ngarurampamba',     'Batteries externes', 'Battery ya Kutoa Umeme'],
        ['radio',                 'Radiyo',                   'Radio', 'Redio'],
        ['home-appliances',       'Ibikoresho by\'urugo',     'Électroménager', 'Vifaa vya Nyumbani'],
        ['gaming',                'Imikino',                  'Jeux vidéo', 'Mchezo'],
        ['printers-scanners',     'Imperemuzi',               'Imprimantes', 'Pirinti na Skana'],
        ['cookware',              'Ibikoresho byo guteka',    'Ustensiles de cuisine', 'Vifaa vya Kuchoma'],
        ['bakeware',              'Ibikoresho byo gukora',    'Moules de cuisson', 'Vifaa vya Kuoka'],
        ['kitchen-knives-tools',  'Amacupa n\'ibikoresho',    'Couteaux de cuisine', 'Viu vya Jikoni'],
        ['pots-pans',             'Ibikapu',                  'Casseroles et poêles', 'Sufuria na Kijiko'],
        ['plates-bowls',          'Amasahani',                'Assiettes et bols', 'Jiko na Baseni'],
        ['cups-mugs',             'Inkomere',                 'Tasses et mugs', 'Kofia'],
        ['cutlery-utensils',      'Ibikoresho byo kurya',     'Couverts', 'Chombo'],
        ['kitchen-storage',       'Ibibiko by\'igikoni',      'Rangement cuisine', 'Hifadhi ya Jikoni'],
        ['blenders-mixers',       'Gakondo',                  'Mixeurs', 'Blenda'],
        ['coffee-tea-makers',     'Igikoresho cy\'icyayi',    'Cafetières et théières', 'Vifaa vya Kuchai Cha Kahawa'],
        ['food-containers',       'Ibibiko by\'ibiribwa',     'Boîtes alimentaires', 'Vifaa vya Hifadhi ya Chakula'],
        ['dining-sets',           'Amaseti yo kurya',         'Sets de table', 'Seti za Kuzungumzia'],
        ['kitchen-cleaning',      'Isuku y\'igikoni',         'Nettoyage cuisine', 'Usafi wa Jikoni'],
        ['petrol-cars',           'Imodoka ya peterori',      'Voitures essence', 'Magari ya Petrole'],
        ['diesel-cars',           'Imodoka ya mazutu',        'Voitures diesel', 'Magari ya Mazut'],
        ['hybrid-cars',           'Imodoka yagizwe n\'amashanyarazi na mazutu', 'Voitures hybrides', 'Magari ya Mchanganyiko'],
        ['electric-cars',         'Imodoka ya amashanyarazi', 'Voitures électriques', 'Magari ya Umeme'],
        ['automatic-cars',        'Imodoka automatike',       'Voitures automatiques', 'Magari ya Otomatiki'],
        ['manual-cars',           'Imodoka y\'intoki',        'Voitures manuelles', 'Magari ya Manuali'],
        ['suvs',                  'Imodoka y\'ibiharaga',     'SUV', 'SUV'],
        ['pickup-trucks',         'Imodoka ya kutoa ibintu',  'Camionnettes', 'Tarak'],
        ['minivans-buses',        'Minivani na bisi',         'Minivans et bus', 'Minibasi'],
        ['motorcycles',           'Moto',                     'Motos', 'Pikipiki'],
        ['car-accessories',       'Ibikoresho by\'imodoka',   'Accessoires auto', 'Vifaa vya Magari'],
        ['car-spare-parts',       'Ibikoresho by\'ibisimbura',       'Pièces détachées', 'Vipengele vya Magari'],
        ['water-bottle',          'Icupa y\'amazi',           'Bouteille d\'eau', 'Chupa ya Maji'],
        ['coffee-bottle',         'Icupa y\'ikawa',           'Bouteille à café', 'Chupa ya Kahawa'],
        ['thermos-flasks',        'Termos',                   'Thermos', 'Thermos'],
        ['tumblers',              'Tumblers',                 'Gobelets', 'Tumbler'],
        ['sports-water-bottles',  'Icupa ya siporo',          'Gourdes sport', 'Chupa ya Mchezo'],
        ['glass-bottles',         'Icupa y\'inzobe',          'Bouteilles en verre', 'Chupa ya Glasi'],
        ['kids-drinking-cups',    'Inkombe z\'abana',         'Tasses enfants', 'Kofia za Watoto'],
        ['wine-glasses',          'Verre de vin',             'Verres à vin', 'Glasi ya Waini'],
        ['mugs',                  'Inkombe',                  'Mugs', 'Magi'],
        ['juice-jugs-pitchers',   'Ibikapu by\'amashyushyu',  'Carafes à jus', 'Jaji ya Juisi'],
    ];
    $stmt = $conn->prepare("UPDATE `subcategories` SET name_rw=?, name_fr=?, name_sw=? WHERE slug=?");
    foreach ($t as [$slug, $rw, $fr, $sw]) {
        $stmt->bind_param('ssss', $rw, $fr, $sw, $slug);
        $stmt->execute();
    }
    $stmt->close();
});
